...
**Iteración siguiente**: migrar **tier_yearly_rates** (P4 / módulo #17). Ya hay 12 filas seedeadas en Cloud (2024–2026 · essentials/special · personal/group). Se refactoriza `services/teacherRatesConfig.ts` al mismo patrón que `subjectsService` (repo async + caché + hidratación + optimistic). Mapear `specialist ↔ special` e `individual ↔ personal` en el borde repo. Al cerrar, avanzar a **Users / Profiles** (P1 / módulo #3) que desbloquea todo el resto.
