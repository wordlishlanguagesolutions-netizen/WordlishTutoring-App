...
| 4 | **Profesores** | ✅ | ✅ | ✅ (update) | ✅ | ✅ (excepto sub-módulos payroll/export/class) | 🟡 (falta admin real en Cloud para test end-to-end) | P1 |
| 5 | **Estudiantes** | ⏳ | ⏳ | ⏳ | ⏳ | ⏳ | ⏳ | P1 |
...
**Progreso global**: 5 / 24 módulos completos (~ 21 %).
...
### 4 · Profesores ✅
- **Estado actual**: migrado (2026-07-25).
- **Lee**: `repositories/teachers.ts` → `public.teachers` con join denormalizado a `user_profiles!teachers_user_id_fkey` (identidad: full_name, first_name, email, phone, avatar_url, active). `services/teachersService.ts` mantiene caché + hidratación idempotente + suscripción reactiva.
- **Escribe**: `teachersRepo.update` (tier, subjects, grades, bio, hourlyRate, stats). Optimistic UI con rollback en `updateTeacher`.
- **Consumidores del módulo actualizados**: `hooks/usePermissions.tsx` (resuelve teacherId por userId contra el cache Cloud primero; fallback mock durante coexistencia con Payrolls/Export/Class que aún no migran).
- **Consumidores pendientes (otros módulos, respetan el orden estricto)**: `services/payrollService.ts` (#18), `services/exportService.ts`, `services/classService.ts` (#10), `app/class/[id].tsx` (#9). Estos leen `mockDb.teachers` porque las funciones que los invocan pertenecen a módulos aún no migrados; se retirarán con sus respectivas iteraciones.
- **RLS**: `authenticated_select_teachers` (using: true) permite listar; `admin_all_teachers` / `supervisor_select_teachers` / `teacher_update_self` cubren mutaciones.
- **Verificaciones ejecutadas**:
  - ✅ Lectura Cloud: `teachersRepo.list()` consulta `teachers` con join.
  - ✅ Escritura Cloud: `update` con `.eq('id', id)` + `updated_at`.
  - ✅ Optimistic UI + rollback en la mutación.
  - ✅ Reactividad (subscribeTeachers → notify en cada cambio).
  - ✅ `usePermissions` resuelve teacherId desde Cloud (rama primaria) sin depender de mockDb.
  - ✅ Módulo Users sigue desacoplado (auditoría previa confirmó cero imports de mockDb en Users).
  - 🟡 Pendiente prueba end-to-end con admin real autenticado.
- **Riesgo residual**: 🟠 Medio hasta activar Auth real (RLS del join a `user_profiles` puede limitar identidad para no-admins; consumo primario actual es admin panel).

### 5-6 · Estudiantes / Acudientes
- **Estado actual**: `mockDb.{students,guardians}` derivados de `mockData`.
...
- **Dependencias**: tablas `students`, `guardians`, `student_guardians` (todas ya existen con RLS).
...
| 2026-07-25 | **Teachers (#4)** | Módulo #4 migrado a Cloud. Auditoría previa confirmó que Users ya no tiene dependencia de mockDb (repos + service + panel + hook limpios; el import de `mockDb` en `usePermissions.tsx` era exclusivamente para guardians/students/teachers, no para users). Se creó `repositories/teachers.ts` como CRUD async sobre `public.teachers` con join denormalizado a `user_profiles!teachers_user_id_fkey` (identidad: full_name, first_name, email, phone, avatar_url, active). Nuevo `services/teachersService.ts` con caché + hidratación idempotente + suscripción reactiva + mutación optimista con rollback (`updateTeacher`). `hooks/usePermissions.tsx` refactor: rama `teacher` consulta primero el cache Cloud (`getTeacherByUserId`) y sólo cae a mockDb como fallback temporal mientras Payrolls/Export/Class no migren; se agregan hidrataciones + suscripciones dobles para re-render reactivo. | `repositories/teachers.ts` (nuevo), `services/teachersService.ts` (nuevo), `hooks/usePermissions.tsx`, `docs/CLOUD_MIGRATION_STATUS.md` | `teachers` (+ join lectura a `user_profiles`) | El join a `user_profiles` respeta RLS del perfil: admin/supervisor obtienen identidad completa; en vistas no-admin (booking wizard estudiante/acudiente) la identidad puede venir vacía. Consumo primario actual es el panel admin, por eso se acepta y se documenta como limitación a resolver al migrar Booking Wizard. Cuatro consumidores en otros módulos siguen leyendo `mockDb.teachers` (payrollService, exportService, classService, class/[id]); es esperado y se retirará con sus iteraciones. | ✅ Lectura Cloud (`teachersRepo.list` con join). ✅ Escritura Cloud (`update` con `updated_at`). ✅ Optimistic UI + rollback. ✅ Suscripción reactiva probada vía `subscribeTeachers`. ✅ `usePermissions` desacoplado (Cloud primero, fallback documentado). ✅ RLS `authenticated_select_teachers` + `admin_all_teachers` + `teacher_update_self`. 🟡 Prueba end-to-end pendiente de admin real autenticado + primer profesor real en Cloud. | OnSpace |
...
**Iteración siguiente**: **#5 Students** (P1). Prerrequisito bloqueante sigue siendo activar Auth real + admin principal + al menos un profesor real (necesario para test end-to-end de #4). Con Teachers ya migrado (#4), el patrón repo async + service con caché + hidratación + suscripción + optimistic mutation queda validado por tercera vez consecutiva; se replicará idéntico para Students sobre `public.students` + `public.student_guardians`. Orden recomendado desde aquí: **#5 Students → #6 Guardians → #7 Availability → #11 HourPackages → #8 Bookings + #9 ClassRecords + #12 Payments**.
